...
describe('the indicators a document may declare', () => {
  it('is the schema own list, not one written here', () => {
    ...
    expect([...INDICATOR_TYPES].sort()).toEqual(['ATR', 'EMA', 'HIGHEST', 'LOWEST', 'RSI', 'SMA'])
  })
  ...
  it('knows which of them read one price series and which read the whole candle', () => {
    ...
    // reader picked.
    const priced: IndicatorType[] = ['SMA', 'EMA', 'RSI']
    const whole: IndicatorType[] = ['ATR', 'HIGHEST', 'LOWEST']

    expect(priced.map(takesSource)).toEqual([true, true, true])
    expect(whole.map(takesSource)).toEqual([false, false, false])
  })

...
// SMA, EMA and RSI take a `period` and a price `source`; ATR, HIGHEST and LOWEST take a period
// and **no source**, because they are defined over the whole candle. A form that emitted
// `source` for all six would produce documents the API refuses — `extra="forbid"` on the params
// model — and the hand-written set of "the ones without a source" is a copy that goes stale the
// first time an indicator is added in Python. ADX and Bollinger are next, and both are
// sourceless.
